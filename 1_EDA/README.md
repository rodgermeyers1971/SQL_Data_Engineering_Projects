...
- 📊 **Data Model:** Star schema with fact + dim + bridge tables
...
- 🧠 Core languages: SQL and Python each appear in ~29k job postings, making them the most demanded skills
- ☁️ Cloud platforms: AWS and Azure are critical for modern day engineering roles
...
- **Complex Joins**: Multi-table `INNER JOIN` operations across `job_postings_fact`, `skills_job_dim`, nd `skills_dim`
- **Aggregations**: `COUNT()`, `MEDIAN()`, and `ROUND()` for statistical analysis
- **Filtering**: Boolean logic with `WHERE` clauses and multiple conditions
...
(skills with >= 100 postings)
- **NULL Handlings**: Proper filtering of incomplete records ('salary_year_avg IS NOT NULL)
